package SimpleDataTypes

import (
	"fmt"
	"goBases/Utils"
	"strconv"
)

func Marketplace() {
	// Una juguetería tiene mucho éxito en dos de sus productos: payasos y muñecas.
	// Suele hacer venta por correo y la empresa de logística les cobra por peso de cada paquete así que deben calcular
	// el peso de los payasos y muñecas que saldrán en cada paquete a demanda.
	// Cada payaso pesa 112 g y cada muñeca 75 g.
	// Escribir un programa que lea el número de payasos y muñecas vendidos en el último pedido y calcule el peso total
	// del paquete que será enviado.
	const CLOWNWEIGHT = 75
	const DOLLWEIGHT = 112
	dolls, _ := strconv.ParseInt(Utils.ScannerInput("Say your dolls amount: "), 10, 9)
	clowns, _ := strconv.ParseInt(Utils.ScannerInput("Say your clowns amount: "), 10, 9)
	totalClownWeight := clowns * CLOWNWEIGHT
	totalDollWeight := dolls * DOLLWEIGHT
	totalWeight := totalDollWeight + totalClownWeight
	fmt.Printf("The total weight is: %d", totalWeight)
}
